...
from model.user import UserState
from app.configs import config
from helper.rpc_client import RPCClient
from model.user import UserState, RoleEntity
...
class Oauth2Bearer(HTTPBearer):
    ...
    # staff online hanya mengisinkan toko yang akan memasukkan kode
    ...
    async def decode_credential(self, credentials: str) -> UserState:
        """
        ...
        if self.include_raw:
            return UserState(
                ...
                entity=[
                    # set list of entity role
                    RoleEntity(**entity) for entity in user_entities
                ]
            )

        #return data and role for spesific entity
        return UserState(id=user.id, slug=user.slug, entity=[
            # set list of entity role
            RoleEntity(**entity) for entity in user_entities
        ])
...
include_raw_token = Oauth2Bearer(include_raw=True)
